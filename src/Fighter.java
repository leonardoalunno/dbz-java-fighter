...
public abstract class Fighter {
    ...
    // --- VARIABILI DI STATO (Promosse per modularità) ---
    ...
    public void takeDamage(int amount) {
        // Se è invincibile, ignora totalmente il danno e l'impatto!
    ...
    public void update(KeyHandler keyH, Fighter opponent) {
        ...
        // 3. Controllo KO Avversario (Vittoria)
        ...
        // 3.5 Gestione Hit Stun, Rinculo e Invincibilità
        if (isHit) {
            hitTimer++;

            // Effetto Rinculo: nei primi 5 frame del colpo, viene spinto all'indietro
            if (hitTimer < 5) {
                x += facingRight ? -(int)(4 * scale) : (int)(4 * scale);
                // Evitiamo che esca dallo schermo per colpa del rinculo
                if (x < 0) x = 0;
                if (x > GamePanel.SCREEN_WIDTH - baseWidth) x = GamePanel.SCREEN_WIDTH - baseWidth;
            }

            // Fine dello stordimento (dopo 20 frame) -> Scatta l'invincibilità!
            if (hitTimer > 20) {
                isHit = false;
                isInvincible = true;
                invincibleTimer = 0;
        ...
        // Timer dell'invincibilità (dura 40 frame)
        if (isInvincible) {
            invincibleTimer++;
        ...
        // 4. Lettura Input (Traduzione universale)
        ...
        // 6. Ricariche (Ki Blast ed Energia Speciale)
        ...
        if (!isChargingAura && !isBlocking && !isHit) {
            // 7. Input Attacchi e Teletrasporto
                ...
                if (!hasHit && opponent != null) {
                    ...
                    if (hitbox != null && hitbox.intersects(opponent.getBounds())) {
                        ...
                        if (damage > 0) {
                            ...
                            hasHit = true;
                            if (attackType == 6) {
                                onSpecialAttackHit(opponent); // Crea l'esplosione unica del personaggio!
                            }
                        }
                    }
                }
            ...
            // 9. Aggiornamento Proiettili Universale
            ...
            if (isTeleporting) {
                ...
                // Limite Schermo 16:9 Universale
                if (x > GamePanel.SCREEN_WIDTH - baseWidth) x = GamePanel.SCREEN_WIDTH - baseWidth;
            }
        } else {
    ...
    // --- DISEGNO HUD UNIVERSALE (Metodo rimasto identico a prima) ---
